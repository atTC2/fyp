...
The web interface covers a series of different views. Spring, supporting MVC, allows for controller classes (annotated with \texttt{@Controller}) to load views (JSPs) and pass them model information acquired from services. Forms, where the user has to input some data, allow a model to bind directly to the input fields on the view. When submitted, automatic validation can be done by Spring, and the controller can receive the model object for processing. Uniform Resource Locators (URLs) are bound to specific controller classes. The controllers can also have different methods for interpreting \texttt{GET} and \texttt{POST} requests, for which different parameters can be specified. All supported URLs and request methods, with controller, JSP and function provided are listed in table \ref{table:webservices}. 

\begin{figure}
	\centering
	\includegraphics[width=13cm]{img/extractorie-add.png} \\
...
\begin{figure}[t]
	\centering
	\includegraphics[width=13cm]{img/extractorie-view.png} \\
	\caption[An Example Paper View on the Website]{The view page, with a paper's content being shown (the full key phrase list at the bottom is cropped short).}
...
\begin{figure}[t]
	\centering
	\includegraphics[width=13cm]{img/extractorie-home.png} \\
...
\begin{figure}
	\centering
	\includegraphics[width=13cm]{img/extractorie-search-supportvectormachine.png} \\
