...
CREATE TABLE IF NOT EXISTS `fyp`.`synonym` (
  ...
  UNIQUE INDEX `syn_link_UNIQUE` (`syn_link` ASC),
  PRIMARY KEY (`id`),
  UNIQUE INDEX `id_UNIQUE` (`id` ASC),
  CONSTRAINT `fk_syn`
...
-- -----------------------------------------------------
-- Table `fyp`.`nlp_object`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `fyp`.`nlp_object` ;

CREATE TABLE IF NOT EXISTS `fyp`.`nlp_object` (
  `id` INT NOT NULL AUTO_INCREMENT,
...
ENGINE = InnoDB
COMMENT = 'Stores the processor objects for each stage of paper processing.';
